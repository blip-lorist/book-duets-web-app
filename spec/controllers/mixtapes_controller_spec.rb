...
RSpec.describe MixtapesController, type: :controller do
  ...
  context "when the owner is required" do
    ...
    describe "require_owner" do
      ...
      it "allows you to view your own mixtapes" do
        get :show, id: @mixtape
        expect(response).to render_template("show")
      end

      it "doesn't allow you to view others' mixtapes" do
        get :show, id: 2
        expect(response).to redirect_to(mixtapes_path)
      end
    end
  end

  context "specs without the require_user filter" do
    # Skip the authentication for testing porpoises
    before(:each) do
      MixtapesController.skip_before_filter :require_user
      MixtapesController.skip_before_filter :require_owner
    end
